...
{#if $activeRP}
	<div class="relative">
		...
		{#if showDropdown && $activeRP.branches.length > 1}
			<div class="absolute top-full left-0 mt-1 w-40 bg-surface2 border border-border-custom rounded-md shadow-lg z-50">
				{#each $activeRP.branches as branch}
					<button
						class="block w-full text-left px-3 py-1.5 text-sm hover:bg-surface transition-colors
