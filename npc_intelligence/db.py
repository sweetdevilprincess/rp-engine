...
from .types import (
    CorrectionPair, FeedbackInput, Pattern, BehavioralCategory, Direction,
    BehavioralSignature,
)


class BehavioralPatternDB:
    ...
    def insert_pattern(self, pattern: Pattern) -> str:
        self.conn.execute(
            ...
            (
                pattern.id,
                pattern.category.value if isinstance(pattern.category, BehavioralCategory) else pattern.category,
                pattern.subcategory,
                pattern.description,
    ...
    def log_output(
        self, session_id: str, behavioral_sig: BehavioralSignature,
        pattern_ids: list[str], output_text: str
    ) -> str:
        output_id = str(uuid.uuid4())
        sig_dict = {
            "archetype": behavioral_sig.archetype.value,
            "modifiers": [m.value for m in behavioral_sig.modifiers],
            "trust_stage": behavioral_sig.trust_stage.value,
            "interaction_type": behavioral_sig.interaction_type.value,
            "scene_signals": [s.value for s in behavioral_sig.scene_signals],
        }
        self.conn.execute(
    ...
    def _row_to_pattern(self, row: sqlite3.Row) -> Pattern:
        ...
        return Pattern(
            id=row["id"],
            category=BehavioralCategory(row["category"]),
