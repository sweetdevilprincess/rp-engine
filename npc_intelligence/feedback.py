...
from .types import (
    ...
    BehavioralCategory, Direction, BehavioralSignature,
)
...
class BehavioralFeedbackProcessor:
    def __init__(self, db: BehavioralPatternDB, llm_call: Optional[LLMCallable] = None):
        self.db = db
        self.llm_call = llm_call

    def process(self, feedback: FeedbackInput,
                behavioral_signature: Optional[BehavioralSignature] = None) -> list[str]:
        """
        ...
        if self.llm_call:
            extractions = self._extract_patterns_with_llm(feedback, behavioral_signature)
        else:
    ...
    def _extract_patterns_with_llm(
        ...
        behavioral_sig: Optional[BehavioralSignature]
    ) -> list[ExtractionResult]:
        prompt = self._build_extraction_prompt(feedback, behavioral_sig)
    ...
    def _build_extraction_prompt(
        ...
        if behavioral_sig:
            sig_desc = (f"Archetype: {behavioral_sig.archetype.value}, "
                        f"Trust Stage: {behavioral_sig.trust_stage.value}, "
                        f"Modifiers: {[m.value for m in behavioral_sig.modifiers]}, "
                        f"Interaction: {behavioral_sig.interaction_type.value}, "
                        f"Scene Signals: {[s.value for s in behavioral_sig.scene_signals]}")
...
Return ONLY a JSON array. No explanation outside the JSON."""
    ...
    def _parse_extraction_response(self, response: str) -> list[ExtractionResult]:
        ...
        valid_categories = {c.value for c in BehavioralCategory}
        ...
        for item in parsed:
            if not isinstance(item, dict):
                continue
            category = item.get("category", "self_interest")
    ...
    def _create_new_pattern(self, extraction: ExtractionResult) -> str:
        ...
        pattern = Pattern(
            id=pattern_id,
            category=BehavioralCategory(extraction.category),
