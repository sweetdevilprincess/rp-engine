...
from .types import (
    CorrectionPair, FeedbackInput, Pattern, PatternCategory, Direction,
    TaskSignature,
)
...
class PatternDB:
    ...
    def log_output(
        self, session_id: str, task_sig: TaskSignature,
        pattern_ids: list[str], output_text: str
    ) -> str:
        output_id = str(uuid.uuid4())
        sig_dict = {
            "mode": task_sig.mode.value,
            "register": task_sig.register.value,
            "intensity": task_sig.intensity.value,
            "position": task_sig.position.value,
            "elements": [e.value for e in task_sig.elements],
        }
        self.conn.execute(
